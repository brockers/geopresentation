function d3dashboard(id, fData, proj, bounds){
	...
	function histoGram(fD){
		...
		bars.append("rect")
			...
			.attr("x", (d,i) => {
				var fromProj = proj.fromLatLngToContainerPixel(bounds.getSouthWest()).x
				console.log("X: original", x(d[0]), "fromProj", x(fromProj));
				return x(d[0]);
			})
			//.attr("y", function(d) { return y(d[1]); })
			.attr("y", function(d) { 
				var fromProj = proj.fromLatLngToContainerPixel(bounds.getSouthWest()).y
		...
		function mouseover(d){	// utility function to be called on mouseover.
			...
			if(pC){ pC.update(nD); }
			if(leg) { leg.update(nD); }
		}
		
		function mouseout(d){	 // utility function to be called on mouseout.
			// reset the pie-chart and legend.	  
			if(pC){ pC.update(tF); }
			if(leg){ leg.update(tF); }
		}
